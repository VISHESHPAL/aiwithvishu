...
export default function PostDetail({
  ...
  const renderPhotoSection = () => {
    ...
    return (
      <div
        className={`border ${darkMode ? "border-neutral-800" : "border-slate-200"}`}
      >
          ...
          <div className="w-full flex flex-col gap-6">
            {items.map((item, index) => (
              <div key={index} className="flex flex-col gap-3">
                ...
                {/* Prompt + Copy Button - Side by Side on all screens */}
                <div
                  className={`border p-3 sm:p-4 relative ${
                    darkMode
                ...
                >
                  <div className="flex flex-row justify-between items-start gap-2">
                    ...
                    >
                      <span className="font-bold text-sky-600 dark:text-sky-400 whitespace-nowrap">
                        PROMPT {index + 1} :
                      </span>{" "}
                    ...
                    <button
                      onClick={() =>
                        copyToClipboard(item.prompt, `photo-${index}`)
                      }
                      className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 transition-all duration-200 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-sky-500 rounded ${
                        copiedItems[`photo-${index}`]
                          ? "bg-green-600 text-white shadow-lg shadow-green-500/30 scale-95"
                          : darkMode
                            ? "bg-neutral-700 text-gray-300 hover:bg-neutral-600 hover:scale-105 hover:shadow-lg hover:shadow-neutral-700/50"
                            : "bg-white text-gray-700 hover:bg-sky-50 hover:text-sky-600 hover:scale-105 border border-gray-200 hover:border-sky-300 hover:shadow-md hover:shadow-sky-100"
                      }`}
                      title="Copy Prompt"
                ...
                {index < items.length - 1 && (
                  <div
                    className={`border-t ${darkMode ? "border-neutral-800" : "border-slate-200"} my-2`}
                  ></div>
                )}
              </div>
            ))}
          ...
          {post.photoBtnLink && (
            <div className="mt-6 w-full flex justify-start">
              <a
                ...
                className="inline-flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs uppercase px-5 py-2.5 transition-all duration-200 m-auto rounded hover:scale-105 hover:shadow-lg hover:shadow-sky-500/30 focus:outline-none focus:ring-2 focus:ring-sky-500"
  ...
  // ✅ Render Video Section - with Interactive Copy Button
    ...
    return (
      ...
      >
        <div className="p-4 sm:p-5 flex items-center justify-center flex-col">
          ...
          {/* Video Prompt + Copy Button - Side by Side */}
          <div
            className={`border p-3 sm:p-4 relative ${
              darkMode
                ? "border-neutral-700 bg-neutral-800/50"
          ...
          >
            <div className="flex flex-row justify-between items-start gap-2">
              ...
              >
                ...
                <span className="break-words">{post.videoPrompt}</span>
              </p>
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 transition-all duration-200 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-sky-500 rounded ${
                  copiedItems["video"]
                    ...
                    : darkMode
                      ? "bg-neutral-700 text-gray-300 hover:bg-neutral-600 hover:scale-105 hover:shadow-lg hover:shadow-neutral-700/50"
                      : "bg-white text-gray-700 hover:bg-green-50 hover:text-green-600 hover:scale-105 border border-gray-200 hover:border-green-300 hover:shadow-md hover:shadow-green-100"
                }`}
                title="Copy Prompt"
          ...
          {post.videoBtnLink && (
            <div className="mt-4">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold text-xs uppercase px-5 py-2.5 transition-all duration-200 rounded hover:scale-105 hover:shadow-lg hover:shadow-green-500/30 focus:outline-none focus:ring-2 focus:ring-green-500"
    ...
    <div className="w-full font-sans">
      ...
      >
        ...
        <div className="p-4 sm:p-5 md:p-6 pt-5 sm:pt-6 md:pt-7 flex flex-col gap-4 sm:gap-5 md:gap-6">
          ...
          <div>
            <h1
              className={`text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl font-black leading-tight ${
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] sm:text-[11px] font-medium text-gray-500 dark:text-gray-400 mt-3 pb-4 border-b border-slate-100 dark:border-neutral-800">
              ...
              >
                <Calendar
                  className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-sky-600"
                  aria-hidden="true"
                />
                <span>{post.date}</span>
              </span>
          ...
          <div>
            <h2
              className={`text-xs font-black uppercase tracking-widest border-b-2 pb-1.5 mb-3 inline-block ${
                darkMode
          ...
          {post.faqs && post.faqs.length > 0 && (
            <div>
              <h2
                className={`text-xs font-black uppercase tracking-widest border-b-2 pb-1.5 mb-4 inline-block ${
                  darkMode
                    ? "text-white border-neutral-700"
                    : "text-black border-black"
                }`}
              ...
              <div className="flex flex-col gap-4">
                {post.faqs.map((faq, idx) => (
                  ...
                  >
                    <h4
                      className={`font-bold text-xs sm:text-sm flex items-start gap-2 ${
                        darkMode ? "text-white" : "text-black"
                      }`}
                      itemProp="name"
                    >
                    ...
                    <p
                      className={`text-xs sm:text-sm mt-2 pl-5 leading-relaxed ${
                        darkMode ? "text-gray-300" : "text-gray-700"
                      }`}
                      itemScope
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="pt-4 border-t border-slate-150 dark:border-neutral-800 flex flex-wrap items-center gap-2">
              <span
                className={`text-xs font-bold flex items-center gap-1 mr-1 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          {relatedPosts.map((rPost) => (
            ...
            >
              <div
                className="relative w-full"
                style={{ paddingBottom: "65%", minHeight: "180px" }}
              >
                <img
                  src={rPost.image}
                  alt={rPost.title}
              ...
              <div className="p-3 sm:p-4 flex-1 flex flex-col gap-2 justify-between">
                <div>
                  ...
                  <p
                    className={`text-[10px] mt-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                  >
      ...
      >
        <h3
          className={`text-base font-black uppercase tracking-wide mb-6 ${
            darkMode ? "text-white" : "text-black"
          }`}
        >
          Leave a Reply
          ...
          <div className="mb-8 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-4 max-h-96 overflow-y-auto pr-1">
              {post.comments.map((comm) => (
                ...
                >
                  <div
                    className={`flex justify-between items-center mb-1 font-bold ${
                      darkMode ? "text-white" : "text-black"
                    }`}
                  >
        ...
        <form onSubmit={handleCommentSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label
              className={`text-xs font-bold ${darkMode ? "text-gray-300" : "text-gray-700"}`}
              htmlFor="comment"
            >
              Comment *
            </label>
